...
public:
	...
	float calculateCorrelation()
	{
		...
		float numerator = 0;
		for (int i = 0; i < N; i++)
			numerator += (x[i]-med_x) * (y[i]-med_y);
		float denominator = N - 1;


		numerator = sum_xy - (sum_x * sum_y) / N;

		float SSxx = sum_x_square- (sum_x*sum_x)/N;
		float SSyy = sum_y_square - (sum_y * sum_y) / N;
		denominator = sqrt(SSxx*SSyy);
	...
	// Function to take input from the dataset from a CSV file
	...
	// Function to take input from the dataset from a matrix of size tam x 2
	void takeInput(float mat[][2], int tam) {
		for (int i = 0; i < tam; i++) {
			float xi = mat[i][0];
			float yi = mat[i][1];
